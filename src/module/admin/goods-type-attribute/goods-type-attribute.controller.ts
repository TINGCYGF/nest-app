...
export class GoodsTypeAttributeController {
  ...
  async doAdd(@Body() body, @Response() res) {
    ...
    if (result) {
      await this.toolsService.success(res, `/${Config.adminPath}/goodsTypeAttribute?id=${body.cate_id}`);
    } else {
      await this.toolsService.error(res, '增加失败', `/${Config.adminPath}/goodsTypeAttribute?id=${body.cate_id}`);
    }
  }
  ...
  async delete(@Query() query,@Response() res) {
    ...
    await this.toolsService.success(res, `/${Config.adminPath}/goodsTypeAttribute?id=${query.cate_id}`);
  }
